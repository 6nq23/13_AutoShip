  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (next.status === "completed") { const warningCount = next.shipped.filter((item) => item.warning).length; setOrders(next.failed.map((item) => item.orderNumber)); setNotice({ kind: next.failed.length || warningCount ? "error" : "ok", text: `Batch complete: ${next.shipped.length} shipped, ${next.failed.length} failed${warningCount ? `, ${warningCount} shipped with a warning` : ""}.` }); return; }
...
function LabelDownloadButton({ labelUrl, label = "All successful labels", compact = false, secondary = false }: { labelUrl: string; label?: string; compact?: boolean; secondary?: boolean }) {
  ...
  return compact
    ? <a href={absoluteUrl} target="_blank" rel="noopener" aria-label={`${label} — open in a new page`} title={`${label} — open in a new page`}><Download /></a>
    : <a className={`button ${secondary ? "secondary" : "primary"}`} href={absoluteUrl} target="_blank" rel="noopener"><Download /> {label}</a>;
}

const isPickupRecovery = (item: { warningCode?: string }) => ["PICKUP_ALREADY_SCHEDULED", "PICKUP_ALREADY_PENDING"].includes(item.warningCode || "");
...
function PickupLabelButton({ batchId, labelUrl, compact = false }: { batchId: string; labelUrl?: string | null; compact?: boolean }) {
  ...
  async function generate() {
    ...
    try {
      ...
      setGeneratedUrl(result.labelUrl);
      if (labelWindow) labelWindow.location.replace(result.labelUrl);
      else setError("Pickup labels are ready. Tap the label button once more to open them.");
    } catch (cause) {
      labelWindow?.close(); setError(cause instanceof Error ? cause.message : "Pickup-scheduled labels could not be generated.");
    } finally { setLoading(false); }
  }
  if (generatedUrl) return <LabelDownloadButton labelUrl={generatedUrl} label="Pickup pending/scheduled labels" compact={compact} secondary />;
  return compact
...
function JobProgress({ job }: { job: ShippingJob }) {
  ...
  const activities = job.orderNumbers.map((orderNumber) => {
    ...
    if (shipped) return { orderNumber, state: shipped.warning ? "warning" : "success", label: shipped.warning || `Shipped with ${shipped.courier}`, icon: shipped.warning ? <AlertTriangle /> : <Check /> };
...
function LegacySupportPage() {
  ...
  return <>
    ...
    <section className="card support-panel ticket-panel conversation-panel"><div className="support-panel-title"><div><TicketCheck /><span><h2>Escalated tickets</h2><p>Refund, return, missing-item, and recovery work</p></span></div></div><div className="ticket-filters" aria-label="Ticket filter">{(["open", "resolved", "all"] as const).map((filter) => <button className={ticketFilter === filter ? "active" : ""} onClick={() => setTicketFilter(filter)} key={filter}>{filter}</button>)}</div>{tickets.length ? <div className="ticket-list">{tickets.map((ticket) => <article className="support-ticket" key={ticket.ticketId}><header><span className={`ticket-status ${ticket.status}`}>{ticket.status}</span><time>{new Date(ticket.createdAt).toLocaleString([], { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}</time></header><strong>{ticket.orderNumber || "Order not identified"}</strong>{!ticket.orderNumber && <p className="ticket-context" style={{color: "var(--orange-dark)", fontWeight: 600, fontSize: "11px", background: "#fff3ee", padding: "6px 10px", borderRadius: "6px", margin: "8px 0"}}>Needs attention: Ask the customer for their order ID.</p>}<p>{ticket.description || friendlyIntent("refund_return")}</p><div><span>{formatPhone(ticket.phone)}</span><span>{ticket.category}</span></div><div style={{display: "flex", gap: "8px"}}><button className="button secondary" style={{width: "100%"}} onClick={() => void setTicketStatus(ticket.ticketId, ticket.status === "open" ? "resolved" : "open")}>{ticket.status === "open" ? <><Check /> Mark resolved</> : <><RefreshCw /> Reopen</>}</button>{ticket.orderNumber && <a className="button secondary" style={{width: "100%", justifyContent: "center", display: "flex"}} href={`https://app.nimbuspost.com/orders?search=${ticket.orderNumber}`} target="_blank" rel="noreferrer">Open in Nimbus</a>}</div></article>)}</div> : <SupportEmpty icon={<TicketCheck />} title={`No ${ticketFilter === "all" ? "" : `${ticketFilter} `}tickets`} detail="Escalations created by the bot will appear here." />}</section>
  </>;
}
...
function HistoryPage() {
  ...
  return <><div className="page-heading"><div><p className="eyebrow">Operations</p><h1>Shipping history</h1><p>Every batch, label, and exception in one place.</p></div></div><section className="card history-card">{loading ? <div className="empty-state"><LoaderCircle className="spin" />Loading batches…</div> : items.length ? items.map((item) => { const pickupWarnings = item.shipped.filter(isPickupRecovery).length; return <div className="history-row" key={item.batchId}><div className="history-icon"><PackageCheck /></div><div><strong>{new Date(item.createdAt).toLocaleDateString(undefined, { day: "numeric", month: "short", year: "numeric" })}</strong><small><Clock3 /> {new Date(item.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })} · by {item.shippedBy}</small>{pickupWarnings > 0 && <small className="history-warning"><AlertTriangle /> {pickupWarnings} pickup-pending/scheduled warning{pickupWarnings === 1 ? "" : "s"}; counted as successful</small>}</div><div className="history-metrics"><span className="success-text">{item.totalShipped} shipped</span><span className={item.totalFailed ? "error-text" : "muted"}>{item.totalFailed} failed</span></div><div className="history-actions">{item.labelUrl && <LabelDownloadButton labelUrl={item.labelUrl} compact />}{pickupWarnings > 0 && <PickupLabelButton batchId={item.batchId} labelUrl={item.pickupScheduledLabelUrl} compact />}{item.failed.length > 0 && <button onClick={() => downloadFailureReport(item)} aria-label="Download errors" title="Download errors"><AlertTriangle /></button>}{item.logs?.length ? <button onClick={() => downloadLogs(item.logs!, item.batchId)} aria-label="Download logs" title="Download logs"><Clock3 /></button> : null}</div></div>; }) : <div className="empty-state"><History /><strong>No shipping history yet</strong><span>Your completed batches will appear here.</span></div>}</section></>;
}
